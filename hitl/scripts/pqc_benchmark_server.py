#!/usr/bin/env python3
# ==============================================================================
# Kyber-6G Hardware-in-the-Loop (HITL) gNodeB Base Station MEC Server
# - Parallel Cryptographic Processing (ThreadPoolExecutor)
# - Level-5 Post-Quantum Hybrid Handshake (ML-KEM-1024 + X25519)
# - ML-DSA-87 Mutual Authentication with Trusted UAV Registry
# - Cached Rapid Rekeying (1-RTT) Key Evolution Engine
...
def main():
    print("=" * 75)
    print(f"[*] 6G gNodeB MEC Base Station Server (UDP) on {HOST}:{PORT}")
    ...
    # Pre-provisioned Trusted UAV Public Key Registry
    ...
    while True:
        try:
            ...
            if msg_type == 0x01:
                ...
                # Verify UAV identity binding in trusted registry
                if ue_id not in TRUSTED_UAV_REGISTRY:
                    # Register verified drone public key on initial authorized enrollment
                    TRUSTED_UAV_REGISTRY[ue_id] = drone_sig_pk
                    print(f"[+] [ENROLL] Enrolled new UAV identity: {ue_id.decode('ascii', errors='ignore').strip()}")
                elif TRUSTED_UAV_REGISTRY[ue_id] != drone_sig_pk:
                    print(f"[-] [AUTH ERROR] Public key mismatch for UAV {ue_id}! Handshake rejected.")
                    continue
                
                # ML-DSA Mutual Verification with Bound Transcript
                ...
                # Master Key Derivation (Level-5 Hybrid HKDF-SHA256)
            ...
            # Message 3: Cached Rapid Rekey (1-RTT) Mode
                ...
                if status == "HIT":
                    # Derive next generation evolved session key with domain separation
                    ...
                    send_framed_udp(sock, client_addr, 0x04, b"CACHED_REKEY_ACK")
                else:
            ...
            # Message 6: AES-256-GCM Telemetry Packet
                ...
                try:
                    ...
                    print(f"[-] [DECRYPT ERROR] AES-GCM decryption failed: {e}")
                    continue
            ...
            print(f"[-] Unexpected server loop error: {e}", file=sys.stderr)
